...
export function DocumentLibrary({ documents, onSelectDoc, onDeleteDoc }: DocumentLibraryProps) {
  ...
  return (
    <div className="space-y-4 h-full flex flex-col px-2 md:px-6 py-4 bg-[#151a26]">
      {/* Header Bar */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-5 rounded-2xl neu-flat">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="px-2.5 py-0.5 text-[10px] font-mono font-bold bg-blue-500/10 text-blue-400 rounded-full">
        ...
        <div className="relative w-full sm:w-64">
          ...
          <input
            ...
            className="w-full neu-pressed rounded-xl pl-9 pr-4 py-2 text-xs outline-none focus:ring-1 focus:ring-blue-500 text-slate-200"
          />
        </div>
      </div>
          ...
          filteredDocs.map((doc, idx) => (
            ...
            >
              <div className="flex items-center gap-3.5 min-w-0">
                <div className="w-10 h-10 rounded-xl neu-pressed flex items-center justify-center shrink-0 text-blue-400">
                  <FileText className="w-5 h-5" />
                </div>
                <div className="min-w-0">
                  <h4 className="text-xs md:text-sm font-bold text-slate-100 truncate group-hover:text-blue-300 transition-colors">
                    {doc.name}
                  </h4>
              ...
              <div className="flex items-center gap-2 shrink-0">
                {onDeleteDoc && (
                  <button
                    ...
                    className="p-2 rounded-lg neu-btn-secondary text-slate-400 hover:text-rose-400 transition-all cursor-pointer"
